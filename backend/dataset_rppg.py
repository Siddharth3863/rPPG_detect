...
import random
import cv2
import avi_to_frames as av

def load_img(filepath, scale,video_length):
    # av.convert(filepath, scale)
    #print(filepath)
    filepath = os.path.join(filepath,'mtcnn_resized')
    ...
    for i in range(begin, begin + (video_length*4)):
        img=cv2.imread(os.path.join(filepath,'{0}.png'.format(i)))
...
def load_img_test(file_path, scale):
    av.convert(file_path, scale)
    #print(filepath)
    filepath = os.path.join(file_path,'frames')
    list = os.listdir(filepath)
    imglen=len(list)
    # begin=random.randrange(0,int(imglen-(video_length*4)))
    whole_video=[]
    ...
    for i in range(imglen):
        img=cv2.imread(os.path.join(filepath,'frame-{0}.png'.format(i)))
        #img=cv2.resize(img,(scale, scale))
        whole_video.append(img)
    whole_video=np.array(whole_video)
...
class DatasetFromFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform:
            input = torch.tensor([self.transform(j).detach().numpy() for j in input],dtype=torch.float32)
            positive1 = torch.tensor([self.transform(j).detach().numpy() for j in positive1],dtype=torch.float32)
            positive2 = torch.tensor([self.transform(j).detach().numpy() for j in positive2],dtype=torch.float32)
            neighbor1 = torch.tensor([self.transform(j).detach().numpy() for j in neighbor1],dtype=torch.float32)
            neighbor2 = torch.tensor([self.transform(j).detach().numpy() for j in neighbor2],dtype=torch.float32)
            neighbor3 = torch.tensor([self.transform(j).detach().numpy() for j in neighbor3],dtype=torch.float32)
        ratio_array=[]
...
class DatasetFromFolderTest(data.Dataset):
    def __init__(self, file_list,num_negative,transform=None,scale=64):
    ...
    def __getitem__(self, index):
        input, file_groundtruth = load_img_test(self.video_list[index],self.scale)
